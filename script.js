...
let userPerPage = 10;
let totalPages = Math.ceil(noOfusers/userPerPage);
...
let liTagPrevious = document.createElement("li");
liTagPrevious.className = "page-item";
ulTag.append(liTagPrevious);

let anchorTagPrevious = document.createElement("a");
anchorTagPrevious.className = "page-link";
function previous(j) {
if (j != 0) {
  anchorTagPrevious.setAttribute("href", "javascript:page(" + j + ");previous(" +(j-1)+ ");");
}
}
anchorTagPrevious.innerHTML = "Previous";
liTagPrevious.append(anchorTagPrevious);

for (let i = 1; i <= totalPages; i++) {
let liTagNum = document.createElement("li");
liTagNum.className = "page-item";
ulTag.append(liTagNum);

let anchorTagNum = document.createElement("a");
anchorTagNum.className = "page-link";
anchorTagNum.setAttribute(
  "href",
  "javascript:page(" + i + "); javascript:next(" + (i + 1) + "); previous(" + (i - 1) + ");"
);
anchorTagNum.innerHTML = "" + i + "";
liTagNum.append(anchorTagNum);
}


let liTagNext = document.createElement("li");
liTagNext.className = "page-item";
ulTag.append(liTagNext);

let anchorTagNext = document.createElement("a");
anchorTagNext.className = "page-link";
function next(j) {
if (j != (totalPages+1)) {
  anchorTagNext.setAttribute("href", "javascript:page(" + j + ");previous(" +(j + 1)+ ");");
}
}
anchorTagNext.innerHTML = "Next";
liTagNext.append(anchorTagNext);
...
eachUser.className = "each-user";
  eachUser.innerHTML = `
  <h3>Name: ${users[i].name}</h3>
  <h4>Email: ${users[i].email}</h4>
  `;
  userList.append(eachUser);
anchorTagNext.setAttribute("href","javascript:next(1);");
}
...
for (let i = (j - 1) * userPerPage; i < (j - 1) * userPerPage + userPerPage; i++) {
  
  anchorTagNext.setAttribute("href","javascript:next("+ (j+1) +");");
  anchorTagPrevious.setAttribute("href","javascript:previous("+ (j-1) +");");
...
document.body.append(container,navTag);